...
const AnniversaryListTitle = GetEnvironmentVariable('AnniversaryListTitle');
...
module.exports = async function (context, myTimer) {
    ...
    if (myTimer.isPastDue) {
        context.log('User Anniversarys Sync is running late!');
    }
    context.log(`User Anniversarys Sync started at ${timeStamp}`);
    try {
        // onInit function
        await onInit(context, myTimer);
        const result = await ensureAnniversarysList();
        // Read Users
        if (result) {
            await getAllUsers(_deltaLink);
        }
        else {
            context.log(`Error Create or Access Anniversary List ${timeStamp}`);
            // process.exit(1);
        }
        // End
        context.log(`User Anniversarys Sync ended at ${timeStamp}`);
        process.exit(0);
    }
    catch (error) {
        context.log(error);
...
// Get Anniversary of User
async function GetUsersAnniversary(userId) {
    let results = null;
    const options = {
        method: 'GET',
        headers: {
        ...
        uri: `https://graph.microsoft.com/v1.0/users/${userId}/Anniversary`,
        json: true
    };
    try {
        results = await request(options);
    } catch (e) {
        _context.log(`error getting Anniversary date for user ${userId}`)
    }
    ...
    try {
        ...
        for (const user of _users.value) {
            ...
            try {
                ...
                _Anniversary = await GetUsersAnniversary(user.id);
                const _year = moment(_Anniversary.toString()).format('YYYY');
                // The Anniversary Date has year 2000
                if (_year === '2000') {
                    // check if user exists
                    _exists = await checkUserExist(user);
                    if (!_exists) {
                        // Add user to List
                        await addUser(user, _Anniversary)
                    } else {
                        //Update user
                        await updateUser(user, _Anniversary)
                    }
                }
            } catch (error) {
        ...
        try {
            ...
            if (_deltaLink) {
                ...
                await _appCatalog.setStorageEntity("UserAnniversaryDeltaLink", _deltaLink, "Users Sync Delta Token");
            }
...
async function onInit(context, myTimer) {
    ...
    try {
        ...
        const _deltaLinkValue = await _appCatalog.getStorageEntity('UserAnniversaryDeltaLink');
...
// Add User to SP List
async function addUser(user, _Anniversary) {
    let _item = null;
    try {
        _item = await sp.web.lists.getByTitle(AnniversaryListTitle).items.add({
            Title: user.displayName,
            jobTitle: user.jobTitle ? user.jobTitle : '',
            email: user.mail ? user.mail : '',
            userAADGUID: user.id,
            Anniversary: moment(_Anniversary.toString()).format('YYYY-MM-DD')
        });
    } catch (e) {
...
// Update User Data
async function updateUser(user, _Anniversary) {
    let _userUpdated = null;
    const _item = await sp.web.lists.getByTitle(AnniversaryListTitle).items.top(1).filter(`userAADGUID eq '${user.id}'`).get()

    if (_item && _item.length > 0) {
        _userUpdated = await sp.web.lists.getByTitle(AnniversaryListTitle).items.getById(_item[0].Id).update({
            Title: user.displayName,
            jobTitle: user.jobTitle ? user.jobTitle : '',
            email: user.mail ? user.mail : '',
            userAADGUID: user.id,
...
async function checkUserExist(user) {
    let _item = null;
    try {
        _item = await sp.web.lists.getByTitle(AnniversaryListTitle).items.top(1).filter(`userAADGUID eq '${user.id}'`).get()
    } catch (e) {
...
async function deleteUser(user) {
    ...
    try {
        ...
        if (_item && _item.length > 0) {
            try {
                await sp.web.lists.getByTitle(AnniversaryListTitle).items.getById(_item[0].Id).delete();
            } catch (e) {
...
// this method ensures that the Anniversarys lists exists, or if it doesn't exist create it

async function ensureAnniversarysList() {

    let _web = new Web(tenantUrl);
    let result = false;

    try {
        const ensureResult = await _web.lists.ensure(AnniversaryListTitle,
            "Anniversarys 2",
            100,
            true);
        ...
        if (ensureResult.list != null) {
            ...
            if (ensureResult.created) {
                ...
                const AnniversaryFieldAddResult = await ensureResult.list.fields.addDateTime(
                    "Anniversary",
                   );
                await AnniversaryFieldAddResult.field.update({ Title: "Anniversary" });
                ...
                try {
                    ...
                    const AnniversaryField = await ensureResult.list.fields.getByInternalNameOrTitle("Anniversary").get();
                    ...
                    _context.log(`The ${AnniversaryListTitle} list does not match the expected fields definition. error ${e}`);
                    result = false;
                }
            }
        }
        ...
        _context.log(`Failed to create Anniversarys list ${AnniversaryListTitle}.`);
        result = false;
    }
